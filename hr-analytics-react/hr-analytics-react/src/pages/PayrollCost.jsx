  ...
  return (
    <>
      <div className="kpi-grid mb-24">
        <div className="kpi-card"><div className="label">{tt('نمو عدد الموظفين (يناير-أغسطس)')}</div><div className="value">{fmtPct(calc.hcGrowth, true)}</div></div>
        <div className="kpi-card"><div className="label">{tt('نمو تكلفة الرواتب (يناير-أغسطس)')}</div><div className="value">{fmtPct(calc.payrollGrowth, true)}</div></div>
        <div className="kpi-card"><div className="label">{tt('تكلفة الموظف شهريًا (أغسطس)')}</div><div className="value">{fmtSAR(calc.perEmployee[7])}</div></div>
        <div className="kpi-card" style={calc.alert ? { borderColor: 'var(--critical)' } : undefined}>
          <div className="label">{tt('الفجوة بين نمو الرواتب ونمو العدد')}</div>
      ...
      {calc.alert ? (
        <div className="card mb-24" style={{ borderRight: '3px solid var(--critical)', background: 'var(--critical-bg)' }}>
          <div className="flex items-center gap-12">
            ...
            <div>
              <div style={{ fontWeight: 800, color: 'var(--critical)' }}>{tt('تنبيه: نمو الرواتب أعلى بكثير من نمو عدد الموظفين')}</div>
              <div className="small" style={{ color: 'var(--critical)' }}>{tt('الفرق')} {fmtPct(calc.gap)} — {tt('قد يعكس زيادات رواتب، ترقيات، عمل إضافي مرتفع، أو تصنيف خاطئ للبيانات. يستحق مراجعة يدوية.')}</div>
            </div>
          </div>
        </div>
      ) : null}
      ...
      <div className="grid-2 mb-24">
        <div className="card">
          <div className="section-head"><h2>{tt('نمو الرواتب مقابل نمو عدد الموظفين (مؤشر = 100 في يناير)')}</h2></div>
        ...
        <div className="card">
          <div className="section-head"><h2>{tt('تكلفة الموظف الشهرية')}</h2></div>
          <LineChart labels={calc.months.map(monthName)} datasets={[{ label: 'تكلفة الموظف', data: calc.perEmployee, color: '#A9803E' }]} height={280} />
        </div>
      </div>

      <div className="card">
        <div className="section-head"><h2>{tt('تفصيل النمو حسب الشركة')}</h2></div>
