  ...
  return (
    <>
      ...
      <div className="grid-2 mb-24">
        <div className="card">
          <div className="section-head"><h2>{tt('اتجاه عدد الموظفين والرواتب')}</h2><span className="hint">{tt('يناير – أغسطس 2026')}</span></div>
          <DualAxisTrendChart
        ...
        <div className="card">
          <div className="section-head"><h2>{tt('مقارنة الشركات — صافي الرواتب (أغسطس)')}</h2></div>
          <BarChart
      ...
      <div className="section">
        <div className="section-head">
          <h2>{tt('أهم ملاحظات الموارد البشرية')}</h2>
          <span className="hint">{tt('مستخرجة تلقائيًا من بيانات أغسطس 2026 مقارنة بالأشهر السابقة')}</span>
        </div>
        <div className="grid-3">
          {insights.map((i, idx) => (
            <div className="insight" key={idx}>
              <div className="top"><div className="title">{tt(i.title)}</div><RiskBadge level={i.severity} /></div>
              <div className="desc">{tt(i.desc)}</div>
              <div className="meta">{tt(i.scope)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="section-head"><h2>{tt('ترتيب الفروع حسب عدد الموظفين')}</h2><a href="#/branches" className="btn ghost sm">{tt('عرض الكل')}</a></div>
          <BarChart
        ...
        <div className="card">
          <div className="section-head"><h2>{tt('حالات تحتاج مراجعة')}</h2><a href="#/review-center" className="btn ghost sm">{tt('مركز المراجعة')}</a></div>
          <div className="flex" style={{ flexDirection: 'column', gap: 10 }}>
            {HR_DATA.overlap_cases.length === 0 ? <EmptyState icon="check-list">{tt('لا توجد حالات تداخل حاليًا')}</EmptyState> : HR_DATA.overlap_cases.map((o, idx) => (
              <div className="flex justify-between items-center" key={idx} style={{ padding: '10px 12px', background: 'var(--paper)', borderRadius: 8 }}>
                <div>
                  ...
                  <div className="small text-muted">{o.company_1_label} ↔ {o.company_2_label} · {tt('تداخل')} {o.overlap_months_names}</div>
                </div>
