...
export default function DataQuality() {
  ...
  const cols = [
    { key: 'rule', label: 'المشكلة', render: (r) => tt(RULE_LABELS[r.rule] || r.rule) },
    ...
    <>
      <div className="grid-2 mb-24">
        <div className="card flex items-center gap-16">
          ...
          <div>
            <div className="small text-muted">{tt('متوسط درجة جودة البيانات')}</div>
            <div style={{ fontSize: 22, fontWeight: 900 }}>{avgScore} / 100</div>
            <div className="small text-muted">{tt('قبل أي تنظيف تلقائي إضافي (Raw Score)')}</div>
          </div>
        </div>
        <div className="card">
          <div className="section-head"><h2>{tt('الدرجة حسب الشركة')}</h2></div>
      ...
      <div className="kpi-grid mb-24">
        {Object.entries(summary).map(([rule, count]) => (
          <div className="kpi-card" key={rule}>
            <div className="label">{tt(RULE_LABELS[rule] || rule)} <Badge tone={RULE_SEVERITY[rule]}>{RULE_SEVERITY[rule]}</Badge></div>
      ...
      <div className="card">
        <div className="section-head"><h2>{tt('سجل المشاكل (عينة حتى 500 من')} {Object.values(HR_DATA.quality_summary).reduce((a, b) => a + b, 0)})</h2></div>
