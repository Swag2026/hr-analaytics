  ...
  return (
    <>
            ...
            <a key={c.company} className="kpi-card" href={`#/branches?company=${c.company}`} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="small text-muted">{c.branches_count} {tt('فرع')}</div>
                </div>
                <ScoreRing score={score} size={54} />
              </div>
              <div className="flex justify-between">
                <div><div className="small text-muted">{tt('الموظفون')}</div><div style={{ fontWeight: 800 }}>{fmtNum(c.headcount)}</div></div>
                <div><div className="small text-muted">{tt('صافي الرواتب')}</div><div style={{ fontWeight: 800 }}>{fmtSAR(c.total_net_payroll)}</div></div>
              </div>
      ...
      <div className="card mb-24">
        <div className="section-head"><h2>{tt('جدول المقارنة الشامل')}</h2><span className="hint">{tt('اضغط على أي شركة للانتقال إلى فروعها')}</span></div>
        <DataTable
          columns={cols}
      ...
      <div className="grid-2">
        <div className="card">
          <div className="section-head"><h2>{tt('عدد الموظفين حسب الشركة')}</h2></div>
        ...
        <div className="card">
          <div className="section-head"><h2>{tt('متوسط الراتب الأساسي')}</h2></div>
