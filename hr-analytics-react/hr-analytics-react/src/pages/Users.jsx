...
function UserForm({ initial, onSubmit, onCancel, busy, isEdit }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {!isEdit ? (
        <div>
          <label className="small text-muted">{tt('اسم المستخدم')}</label>
      ...
      <div>
        <label className="small text-muted">{tt('الاسم الكامل')}</label>
        <input value={form.full_name || ''} onChange={set('full_name')} style={inputStyle} />
      </div>
      <div>
        <label className="small text-muted">{tt('الصلاحية')}</label>
        <select value={form.role} onChange={set('role')} style={inputStyle}>
          <option value="viewer">{tt('مستعرض — عرض البيانات فقط')}</option>
          <option value="admin">{tt('مشرف — كل الصلاحيات بما فيها إدارة المستخدمين')}</option>
        </select>
      </div>
      <div>
        <label className="small text-muted">{tt(isEdit ? 'كلمة مرور جديدة (اتركها خالية لعدم التغيير)' : 'كلمة المرور')}</label>
        <input
          type="text"
          required={!isEdit}
          value={form.password || ''}
...
export default function Users() {
  ...
  function openCreateModal() {
    openModal(
      <>
        <ModalHead title={tt('إضافة مستخدم جديد')} onClose={closeModal} />
  ...
  function openEditModal(u) {
    openModal(
      <>
        <ModalHead title={`${tt('تعديل:')} ${u.username}`} onClose={closeModal} />
        <div className="modal-body">
          <UserForm
  ...
  const cols = [
    ...
    { key: 'role', label: 'الصلاحية', render: (r) => <Badge tone={r.role === 'admin' ? 'high' : 'neutral'}>{tt(ROLE_LABEL[r.role] || r.role)}</Badge> },
    { key: 'created_at', label: 'تاريخ الإنشاء', render: (r) => new Date(r.created_at).toLocaleDateString('ar-SA') },
    {
      key: 'actions', label: '', render: (r) => (
        <div className="flex gap-8" onClick={(e) => e.stopPropagation()}>
          <button className="btn sm" onClick={() => openEditModal(r)}><Icon name="settings" size={13} /> {tt('تعديل')}</button>
          {r.id !== me?.id ? (
            <button className="btn danger sm" onClick={() => deleteUser(r)}>{tt('حذف')}</button>
          ) : null}
        </div>
      ),
    },
  ...
  return (
    <div className="card mb-16">
      <div className="table-toolbar">
        <div className="small text-muted">{rows.length} {tt('مستخدم')}</div>
        <button className="btn primary sm" onClick={openCreateModal}><Icon name="grid" size={13} /> {tt('إضافة مستخدم')}</button>
      </div>
      <DataTable
        columns={cols}
        rows={rows}
