...
export default function Payroll() {
  ...
  const regCols = [
    ...
    { key: 'status', label: 'الحالة', render: (r) => <span className={`badge ${r.status === 'أساسي' ? 'low' : 'medium'}`}>{tt(r.status)}</span> },
  ...
  return (
    <>
      <div className="kpi-grid mb-24">
        <div className="kpi-card"><div className="label">{tt('الراتب الأساسي')}</div><div className="value">{fmtSAR(t.total_basic)}</div></div>
        <div className="kpi-card"><div className="label">{tt('إجمالي البدلات')}</div><div className="value">{fmtSAR(t.total_allowances)}</div></div>
        <div className="kpi-card"><div className="label">{tt('العمل الإضافي')}</div><div className="value">{fmtSAR(t.total_overtime)}</div></div>
        <div className="kpi-card"><div className="label">{tt('إجمالي الاستقطاعات')}</div><div className="value">{fmtSAR(t.total_deductions)}</div></div>
        <div className="kpi-card"><div className="label">{tt('التأمينات (GOSI)')}</div><div className="value">{fmtSAR(t.total_gosi)}</div></div>
        <div className="kpi-card"><div className="label">{tt('السلف')}</div><div className="value">{fmtSAR(t.total_advances)}</div></div>
        <div className="kpi-card"><div className="label">{tt('خصم الغياب')}</div><div className="value">{fmtSAR(t.total_absence_amount)}</div></div>
        <div className="kpi-card"><div className="label">{tt('صافي الرواتب')}</div><div className="value" style={{ color: 'var(--brass-dark)' }}>{fmtSAR(t.total_net)}</div></div>
      </div>

      <div className="grid-2 mb-24">
        <div className="card">
          <div className="section-head"><h2>{tt('اتجاه الرواتب الشهري')}</h2></div>
        ...
        <div className="card">
          <div className="section-head"><h2>{tt('تركيبة الراتب')} ({tt(monthName(m))})</h2></div>
      ...
      <div className="grid-2 mb-24">
        <div className="card">
          <div className="section-head"><h2>{tt('مقارنة الشركات — صافي الرواتب')}</h2></div>
        ...
        <div className="card">
          <div className="section-head"><h2>{tt('مقارنة الفروع — أعلى 10 حسب إجمالي الرواتب')}</h2></div>
      ...
      <div className="card mb-24">
        <div className="section-head"><h2>{tt('توزيع الرواتب الأساسية بين الموظفين')}</h2><span className="hint">{tt(monthName(m))} · {company ? companyLabel(company) : tt('كل الشركات')}</span></div>
        <BarChart labels={distribution.bucketLabels} datasets={[{ label: 'عدد الموظفين', data: distribution.counts, color: '#A9803E' }]} height={260} />
      </div>

      <div className="card">
        <div className="section-head"><h2>{tt('مسير رواتب الموظفين — تفصيل كامل')}</h2><span className="hint">{tt(monthName(m))} · {company ? companyLabel(company) : tt('كل الشركات')} · {tt('كل مكونات الراتب كما وردت في ملفات الرواتب الأصلية')}</span></div>
