  ...
  return (
    <>
      <div className="kpi-grid mb-24">
        <div className="kpi-card"><div className="label">{tt('حالات تحتاج مراجعة')}</div><div className="value">{HR_DATA.overlap_cases.length}</div></div>
        <div className="kpi-card"><div className="label">{tt('إجمالي التعرض المالي المحتمل')}</div><div className="value" style={{ color: 'var(--critical)' }}>{fmtSAR(HR_DATA.overlap_cases.reduce((s, o) => s + o.potential_exposure, 0))}</div></div>
        <div className="kpi-card"><div className="label">{decidedCount ? `${tt('تم اتخاذ قرار في')} ${decidedCount} ${tt('من')} ${HR_DATA.overlap_cases.length} ${tt('حالة')}` : tt('لم تُتخذ أي قرارات بعد')}</div></div>
      </div>

      {err ? <div className="card mb-16" style={{ background: 'var(--critical-bg)', color: 'var(--critical)' }}>{err}</div> : null}

      <div className="section-head mb-16"><h2>{tt('الحالات المفتوحة')}</h2><span className="hint">{tt('مرتبة حسب درجة الخطورة')}</span></div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {sorted.length === 0 ? (
          <EmptyState icon="check-list">{tt('لا توجد حالات مفتوحة حاليًا — كل الحالات المكتشفة تمت مراجعتها')}</EmptyState>
        ) : sorted.map((o) => {
            ...
            <div className="card" key={o.id} style={{ borderRight: `3px solid ${o.risk_level === 'CRITICAL' ? 'var(--critical)' : 'var(--warning)'}` }}>
              <div className="flex justify-between items-start mb-12">
                <div>
                  ...
                  <div className="small text-muted">{tt('رقم الهوية:')} {o.national_id}</div>
                </div>
                <span className="chip">{tt('تداخل:')} {o.overlap_months_names}</span>
              </div>
              ...
              <div className="flex justify-between items-center mb-12" style={{ padding: '10px 14px', background: 'var(--critical-bg)', borderRadius: 8 }}>
                <span className="small" style={{ color: 'var(--critical)', fontWeight: 700 }}>{tt('القيمة المحتملة (غير مؤكدة كخسارة)')}</span>
              ...
              <div className="card" style={{ padding: 12, background: isCrossSponsor ? 'var(--success-bg)' : 'var(--paper)', marginBottom: 4 }}>
                <div className="small" style={{ fontWeight: 700, marginBottom: 4 }}>{tt(isCrossSponsor ? '✓ دليل من ملف المصدر (يرجّح أنها ليست صرفًا مزدوجًا):' : 'الدليل المتاح:')}</div>
                <div className="small text-muted">{o.evidence}</div>
              </div>

              <div className="flex gap-8" style={{ flexWrap: 'wrap', marginTop: 12 }}>
                <button className="btn danger sm" disabled={busy} onClick={() => decide(o.id, 'DOUBLE_PAYMENT')}>{tt('اعتماد صرف مزدوج')}</button>
                <button className="btn sm" disabled={busy} onClick={() => decide(o.id, 'TRANSFER')}>{tt('اعتبارها نقلًا')}</button>
                <button className="btn sm" disabled={busy} onClick={() => decide(o.id, 'DATA_ERROR')}>{tt('اعتبارها خطأ بيانات')}</button>
                <button className="btn ghost sm" disabled={busy} onClick={() => decide(o.id, 'CLOSED')}>{tt('إغلاق الحالة')}</button>
              </div>
              {decision ? (
                <div className="small mb-8" style={{ marginTop: 8 }}>
                  <Badge tone={DECISION_COLORS[decision]}><Icon name="check-list" size={12} /> {tt(DECISION_LABELS[decision])}</Badge>{' '}
                  <span className="text-muted">
                    {tt('بواسطة')} {o.decided_by_name} — {o.decided_at ? new Date(o.decided_at).toLocaleString('ar-SA') : ''}
                  </span>
                </div>
              ) : null}
            </div>
      ...
      {(HR_DATA.resolved_cases || []).length ? (
        <>
          <div className="section-head mb-16 mt-24"><h2>{tt('حالات تم حلّها')}</h2><span className="hint">{tt('قرارات معتمدة سابقًا من المستخدم — محفوظة هنا للتدقيق فقط')}</span></div>
            ...
            {HR_DATA.resolved_cases.map((r, idx) => (
              <div className="flex justify-between items-center" key={idx} style={{ padding: '12px 16px', background: 'var(--success-bg)', borderRadius: 10 }}>
                <div>
                  ...
                  <div className="small" style={{ color: 'var(--success)' }}>{r.previous_company} ← → {r.new_company} · {tt(r.resolution)}</div>
                </div>
                <Badge tone="low">{tt('نقل مؤكد')}</Badge>
              </div>
            ))}
          </div>
        </>
