    ...
    <>
      <div className="table-toolbar mb-16">
        <div className="search-box" style={{ position: 'relative' }}>
          ...
          <input type="text" placeholder={tt('ابحث عن مؤشر...')} style={{ paddingRight: 34, minWidth: 280 }} value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {filtered.map((k) => (
          <div className="card" key={k.name}>
            <div style={{ fontWeight: 900, fontSize: 14.5, color: 'var(--ink)', marginBottom: 10 }}>{tt(k.name)}</div>
            <div className="grid-2" style={{ gap: 20 }}>
              <div>
                <div className="small" style={{ fontWeight: 700, color: 'var(--text-muted)' }}>{tt('التعريف')}</div>
                <div className="small mb-12">{tt(k.def)}</div>
                <div className="small" style={{ fontWeight: 700, color: 'var(--text-muted)' }}>{tt('طريقة الحساب')}</div>
                <div className="mono small mb-12" style={{ background: 'var(--paper)', padding: '8px 10px', borderRadius: 6 }}>{tt(k.formula)}</div>
              </div>
              <div>
                <div className="small" style={{ fontWeight: 700, color: 'var(--text-muted)' }}>{tt('مصدر البيانات')}</div>
                <div className="small mb-12">{tt(k.source)}</div>
                <div className="small" style={{ fontWeight: 700, color: 'var(--text-muted)' }}>{tt('طريقة التفسير')}</div>
                <div className="small">{tt(k.interp)}</div>
              </div>
            </div>
          </div>
        ))}
