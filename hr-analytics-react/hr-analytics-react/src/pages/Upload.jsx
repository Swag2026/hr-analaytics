...
export default function Upload() {
  ...
  function pickFile(f) {
    ...
    if (!f.name.toLowerCase().endsWith('.xlsx') && !f.name.toLowerCase().endsWith('.xlsm')) {
      setError(tt('الملف يجب أن يكون بصيغة xlsx'));
      return;
    }
    setFile(f);
    setError('');
  ...
  async function submit() {
    ...
    try {
      ...
      setError(err.message || tt('تعذر معالجة الملف'));
      setPhase('error');
    }
  }
    ...
    <>
        ...
        <div className="card mb-24">
          ...
          >
            ...
            <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 4 }}>
              {file ? file.name : tt('اسحب ملف Excel هنا')}
            </div>
            <div className="small text-muted mb-16">{tt('أو')}</div>
            <input ref={inputRef} type="file" accept=".xlsx,.xlsm" style={{ display: 'none' }} onChange={(e) => pickFile(e.target.files[0])} />
            <button className="btn primary" onClick={() => inputRef.current.click()}>{tt('اختر ملفًا')}</button>
            <div className="small text-muted" style={{ marginTop: 14 }}>
              {tt('يقرأ النظام الملف فعليًا (ليس محاكاة) — يكتشف الأعمدة تلقائيًا من عناوين الشيت، وأي تحميل لنفس الشركة والشهر يستبدل البيانات السابقة لهما.')}
            </div>
          </div>
          ...
          {file ? (
            <div className="flex gap-8" style={{ marginTop: 16 }}>
              <button className="btn primary" onClick={submit}>{tt('معالجة ورفع الملف')}</button>
              <button className="btn ghost" onClick={reset}>{tt('إلغاء')}</button>
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {phase === 'uploading' ? (
        <div className="card" style={{ textAlign: 'center', padding: 40 }}>
          <div style={{ fontWeight: 800, marginBottom: 8 }}>{tt('جارٍ قراءة الملف وتحليله...')}</div>
          <div className="small text-muted">{tt('اكتشاف الأعمدة، تنظيف القيم، فحص الجودة، والمطابقة مع البيانات الحالية')}</div>
        </div>
      ) : null}

      {phase === 'result' && result ? (
        <div className="card">
          <div className="section-head"><h2>{tt('نتيجة المعالجة:')} {result.filename}</h2></div>
          <div className="kpi-grid mb-16">
            <div className="kpi-card"><div className="label">{tt('الشيت المستخدم')}</div><div className="value" style={{ fontSize: 16 }}>{result.sheet}</div></div>
            <div className="kpi-card"><div className="label">{tt('عدد السجلات')}</div><div className="value">{result.row_count}</div></div>
            <div className="kpi-card"><div className="label">{tt('مشاكل مكتشفة')}</div><div className="value" style={{ color: result.issues.length ? 'var(--warning)' : 'var(--success)' }}>{result.issues.length}</div></div>
            <div className="kpi-card"><div className="label">{tt('تصحيحات تلقائية')}</div><div className="value" style={{ color: 'var(--success)' }}>{result.corrections.length}</div></div>
            <div className="kpi-card"><div className="label">{tt('درجة جودة البيانات')}</div><div className="value">{result.quality_score}<span className="unit">/100</span></div></div>
          </div>

          {result.issues.length ? (
            <div className="mb-16">
              <h4 className="mb-8">{tt('المشاكل المكتشفة')}</h4>
              <DataTable
                columns={[
                  { key: 'row', label: 'الصف' },
          ...
          {result.corrections.length ? (
            <div className="mb-16">
              <h4 className="mb-8">{tt('تصحيحات تلقائية طُبّقت')}</h4>
              <DataTable
                columns={[
          ...
          <button className="btn primary" onClick={reset}>{tt('رفع ملف آخر')}</button>
        </div>
      ) : null}

      <div className="card mt-24">
        <div className="section-head"><h2>{tt('سجل عمليات الرفع')}</h2></div>
        <UploadHistory />
      </div>
    </>
  );
  ...
  return (
    <DataTable
      columns={[
        ...
        { key: 'company', label: 'الشركة', render: (r) => tt(companyLabel(r.company)) },
        { key: 'month_name', label: 'الشهر', render: (r) => tt(r.month_name || monthName(r.month)) },
