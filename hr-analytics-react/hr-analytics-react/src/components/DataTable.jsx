    ...
    <div>
      <div className="table-toolbar">
        {opts.search !== false ? (
          <div className="search-box">
            ...
            <input
              type="text"
              placeholder={isEn ? translateText(opts.searchPlaceholder || 'بحث...') : (opts.searchPlaceholder || 'بحث...')}
        ...
        <div className="flex items-center gap-12">
          <div className="small text-muted">{rows.length} {isEn ? 'records' : 'سجل'}</div>
          {opts.exportFilename ? (
            <button
              className="btn sm"
            ...
            >
              <Icon name="download" size={13} /> {isEn ? 'Download Excel' : 'تحميل Excel'}
            </button>
          ) : null}
        </div>
      </div>
      <div className="table-wrap">
        <div className="table-scroll">
          {filtered.length === 0 ? (
            <EmptyState icon="search">{isEn ? 'No matching results' : 'لا توجد نتائج مطابقة'}</EmptyState>
          ) : (
