        ...
        <div className="modal-body">
          <div className="flex items-center gap-16 mb-24">
            ...
            <div>
              ...
              <div style={{ fontSize: 20, fontWeight: 900 }}>تقييم الفرع: {b.score}/100</div>
              <div className="small text-muted">دوران تقديري {b.turnover_rate_estimate}% · {b.transfers_in} نقل وارد · {b.transfers_out} نقل صادر</div>
            </div>
          </div>
          <div className="grid-3 mb-16">
            <div className="card" style={{ padding: 12 }}><div className="small text-muted">عدد الموظفين</div><div style={{ fontWeight: 800, fontSize: 18 }}>{fmtNum(b.headcount)}</div></div>
            <div className="card" style={{ padding: 12 }}><div className="small text-muted">إجمالي الرواتب</div><div style={{ fontWeight: 800, fontSize: 18 }}>{fmtSAR(b.total_net)}</div></div>
            <div className="card" style={{ padding: 12 }}><div className="small text-muted">متوسط الراتب</div><div style={{ fontWeight: 800, fontSize: 18 }}>{fmtSAR(b.avg_basic)}</div></div>
          </div>
          <div className="grid-3 mb-24">
            <div className="card" style={{ padding: 12 }}><div className="small text-muted">إجمالي أيام الغياب</div><div style={{ fontWeight: 700 }}>{fmtNum(b.total_absence_days)}</div></div>
            <div className="card" style={{ padding: 12 }}><div className="small text-muted">خصم الغياب</div><div style={{ fontWeight: 700 }}>{fmtSAR(b.total_absence_amount)}</div></div>
            <div className="card" style={{ padding: 12 }}><div className="small text-muted">الاقتطاعات</div><div style={{ fontWeight: 700 }}>{fmtSAR(b.total_deductions)}</div></div>
          </div>
          <h4 className="mb-8">اتجاه الفرع الشهري (تكلفة الرواتب المنسوبة لهذا الفرع كل شهر)</h4>
          <div className="mb-16">
            <LineChart
          ...
          <a className="btn primary" href={`#/employees?company=${b.company}&branch=${encodeURIComponent(b.branch)}`}>
            عرض موظفي هذا الفرع <Icon name="chevronLeft" size={14} />
          </a>
        </div>
      </>
    );
    ...
    return (
      <div key={b.company + b.branch} className="flex justify-between items-center" style={{ padding: '9px 12px', background: 'var(--paper)', borderRadius: 8, cursor: 'pointer' }} onClick={() => showBranchModal(b)}>
        <div>
          ...
          <div className="small text-muted">{b.company_label} · {b.headcount} موظف · دوران {b.turnover_rate_estimate}%</div>
        </div>
  ...
  return (
    <>
      <div className="kpi-grid mb-24">
        <div className="kpi-card"><div className="label">عدد الفروع</div><div className="value">{list.length}</div></div>
        <div className="kpi-card"><div className="label">إجمالي الموظفين بالفروع</div><div className="value">{fmtNum(list.reduce((s, b) => s + b.headcount, 0))}</div></div>
        <div className="kpi-card"><div className="label">إجمالي رواتب الفروع</div><div className="value">{fmtSAR(list.reduce((s, b) => s + b.total_net, 0))}</div></div>
        <div className="kpi-card"><div className="label">إجمالي خصم الغياب</div><div className="value">{fmtSAR(list.reduce((s, b) => s + b.total_absence_amount, 0))}</div></div>
        <div className="kpi-card"><div className="label">النقل بين الفروع (يناير-أغسطس)</div><div className="value">{transfers.length}</div></div>
        <div className="kpi-card"><div className="label">متوسط تقييم الفروع</div><div className="value">{Math.round(list.reduce((s, b) => s + b.score, 0) / (list.length || 1))}<span className="unit">/100</span></div></div>
      </div>

      <div className="grid-2 mb-24">
        <div className="card">
          <div className="section-head"><h2>الأفضل أداءً</h2></div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{best.map(rowHtml)}</div>
        </div>
        <div className="card">
          <div className="section-head"><h2>الأقل أداءً / الأكثر احتياجًا للمراجعة</h2></div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{worst.map(rowHtml)}</div>
        </div>
      </div>

      <div className="card mb-24">
        <div className="section-head"><h2>عدد الموظفين حسب الفرع</h2></div>
      ...
      <div className="card mb-24">
        <div className="section-head"><h2>كل الفروع — مع مؤشرات الدوران والغياب</h2><span className="hint">قابل للتصدير Excel للتدقيق</span></div>
      ...
      <div className="card">
        <div className="section-head"><h2>سجل النقل بين الفروع</h2><span className="hint">اكتُشف تلقائيًا من تغيّر الفرع بين شهرين متتاليين لنفس الموظف — كل شهر يُحسب على فرعه الفعلي في ذلك الشهر</span></div>
